      ...
      <div className={styles.container}>
        ...
        {router.pathname === '/' && (
          <BookPreview
            ...
            author={
              selectedBook.author ?? (!errors['author'] && watch('author'))
            }
            imgURL={
              selectedBook.imgURL ?? (!errors['imgURL'] && watch('imgURL'))
            }
            rating={
              selectedBook.rating ?? (!errors['rating'] && watch('rating'))
            }
          />
        )}
        {/* form */}
        <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
          ...
          {(conditionalFields
            ? settings.filter((setting) =>
                conditionalFields.includes(setting.id)
              )
            : settings
          ).map(
          ...
          <div className={styles.buttons}>
            <Button
              type='button'
              text={
                router.pathname === '/' ? 'SKIP FOR NOW' : 'DISCARD CHANGES'
              }
              color='dark'
              size='small'
              onClick={nonSubmit}
            ...
            <Button
              type='submit'
              text={
                router.pathname === '/' ? 'SAVE & CONTINUE' : 'SAVE CHANGES'
              }
              color='light'
              size='small'
            />
